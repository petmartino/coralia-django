...
def calculate_pricing(quote_or_form_data):
    ...
    # ... (code for getting total_people remains the same) ...
    ...
    # ... (code for event_type and weekday logic remains the same) ...
    ...
    # ... (code for all rate calculations remains the same) ...
    ...
    manager_wedding_fee = event_type.has_wedding_fee and event_type.manager_base_fee if event_type.has_wedding_fee else 0.0 # Corrected logic
    ...
    # --- UPDATED: Fetch extra_charge ---
    extra_charge_val = data.extra_charge if is_instance else data.get('extra_charge', 0.0)
    log.append(f"Cargo Extra / Ajuste: ${extra_charge_val:,.2f}")

    breakdown = {
        'cost_musicians_base': total_musician_cost, 'cost_gala_fee': attire_fee * total_people,
        ...
        'extra_charge': extra_charge_val, # Add to breakdown
        # --- UPDATED: Final cost calculation ---
        'total_cost': subtotal - discount_val + extra_charge_val
    }
